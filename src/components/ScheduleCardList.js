...
export const ScheduleCardList = ({ items, day, group }) => {
  ...
  return sessions_data.map(session => (
    ...
    className={"d-block d-xl-table-cell "+`track-bdr-${session.node.data.session__track}`.replace(/[\s&$!]+/g, "")}>
      ...
      <Link style={{ pointerEvents: session.node.data.session__anchor_truncated || session.node.data.url? "auto":"none"}} title={session.node.data.session__title}
        to={ session.node.data.session__anchor_truncated? session.node.data.session__anchor_truncated: session.node.data.url }>
